...
class Model:
    ...
    def build_test_graph(self, input_tensors):
        ...
        with tf.variable_scope('model', reuse=self.get_should_reuse_variables()):
            ...
            outputs, final_states, contexts = self.decode_outputs(target_words_vocab=target_words_vocab,
        ...
        return predicted_indices, topk_values, target_index, attention_weights, outputs.rnn_output, contexts
    ...
    def predict(self, predict_data_lines):
        if self.predict_queue is None:
            ...
            self.predict_top_indices_op, self.predict_top_scores_op, _, self.attention_weights_op, self.embeddings, self.contexts = \
                self.build_test_graph(reader_output)
        ...
        for line in predict_data_lines:
            predicted_indices, top_scores, true_target_strings, attention_weights, path_source_string, path_strings, path_target_string, embeddings, contexts = self.sess.run(
            ...
            all_embeddings.append(contexts)
        ...
        return results, np.array(all_embeddings)
