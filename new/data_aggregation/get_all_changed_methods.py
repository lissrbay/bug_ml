...
from new.data_aggregation.get_java_methods import find_changed_methods


def parse_method_signature(changed_methods: Set[Tuple[str, str]]) -> List[Tuple[str, str]]:
    methods = []
...
def save_results(data_dir: str, issue2commit: Dict[str, str], commit_changed_methods: Dict[str, Set[Tuple[str, str]]]):
    info = dict()
    for issue_id, commit in issue2commit.items():
        changed_methods = commit_changed_methods[commit]
        if changed_methods:
            methods = parse_method_signature(changed_methods)
            info[issue_id] = {"hash": commit, "fixed_methods": methods}
        else:
            info[issue_id] = {"hash": commit, "fixed_methods": []}
